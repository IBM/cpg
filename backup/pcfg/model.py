...
class PCFGModel(nn.Module):
    ...
    def __init__(self, model, y_vocab, x_vocab, word_dim, hidden_value_dim, hidden_type_dim,
        ...
        assert not use_prim_type_oracle, "Primitive type oracle not yet implemented for PCFG"
        self.use_prim_type_oracle = use_prim_type_oracle
        self.dropout = nn.Dropout(dropout_prob)
        self.embedding = nn.Embedding(num_embeddings=self.num_words,
                                      embedding_dim=word_dim)
        self.token_to_type_map = None
        if self.use_prim_type_oracle:
            self.token_to_type_map = torch.zeros([len(self.x_vocab)], dtype=torch.int64)
            for word in pcfg_word_to_type:
                token_idx = self.x_vocab._token_to_idx[word]
                type = pcfg_word_to_type[word]
                self.token_to_type_map[token_idx] = type
        self.encoder = TypedBinaryTreeLSTM(word_dim=word_dim,
                                           ...
                                           is_lstm=model == 'lstm',
                                           scan_token_to_type_map=self.token_to_type_map)
