...
class lstm_seq2seq(nn.Module):
    ...
    def __init__(self, encoder_input_size, decoder_input_size, hidden_size, y_vocab):
        ...
        self.encoder = lstm_encoder(input_size = encoder_input_size, hidden_size = hidden_size)
        self.decoder = lstm_decoder(input_size = decoder_input_size, hidden_size = hidden_size)
    ...
    def train_model(self, input_tensor, target_tensor, n_epochs, target_len, batch_size, training_prediction = 'recursive', teacher_forcing_ratio = 0.5, learning_rate = 0.01, dynamic_tf = False):
        ...
        for it in range(n_epochs):
            pbar = tqdm.tqdm(range(n_batches), colour='blue', desc=f'Epoch {it}')
            ...
            for b in range(n_batches):
                ...
                decoder_input = torch.stack([self.y_vocab.token_to_ohe('<SOS>').float() for _ in range(batch_size)])   # shape: (batch_size, decoder_input_size)
